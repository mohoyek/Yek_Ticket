import type { Ticket, Comment, Attachment, User } from '../env.node.js';
import { NotificationService } from './notification.service.node.js';
import { PushService } from './push.service.node.js';
...
export class TicketService {
  ...
  constructor(private db: any, private uploadDir?: string) {
  ...
  async getStats(
    ...
    return {
      ...
      byStatus,
      byPriority,
      byAssignee: assigneeResult.map((r) => ({
  ...
  async delete(id: number): Promise<void> {
    ...
    if (this.bucket && attachments.length > 0) {
      try {
        await Promise.all(attachments.map((a) => this.bucket!.delete(a.file_url)));
      } catch (error) {
